...
[数据结构模板](https://wu-kan.github.io/posts/acm/template/数据结构)
对蓝书上的懒标记进行加强，将对区间的操作改成区间线性变换，那么区间修改就是区间先乘0在修改，区间加就是区间先乘1再加。这样敲反而简单很多。

> update: 更新了线段树模板。新的模板省去了build操作。
...
```
原：目测是全网最短解法（Vjudge上Length1705，比第二名少了312，同时性能也比较强）。
```cpp
#include<bits/stdc++.h>
using namespace std;
typedef int ll;
